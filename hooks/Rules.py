...
def totalLevelsAcrossAllJobs(state: CollectionState, player: int, target_total: str):
    """Calculate the total levels across all unlocked jobs and check if it meets the target"""
    target_total_int = int(target_total)
    total_levels = 0
    
    # Calculate levels for each job type
    
    # ARR Jobs (start at level 1, get 10 levels free with crystal)
    for job in ["PLD","WAR","DRG","MNK","BRD","BLM","WHM","SMN/SCH","NIN"]:
        if state.has(f"{job} Job Crystal (default cap 10)", player):
            base_level = 10
            level_items = state.count(f"{job} Level Increased by 5", player)
    ...
    # HW Jobs (start at level 30 with crystal)
    for job in ["DRK","MCH","AST"]:
        if state.has(f"{job} Job Crystal (default cap 30)", player):
            base_level = 30
            level_items = state.count(f"{job} Level Increased by 5", player)
    ...
    # STB Jobs (start at level 50 with crystal)
    for job in ["SAM","RDM"]:
        if state.has(f"{job} Job Crystal (default cap 50)", player):
            base_level = 50
            level_items = state.count(f"{job} Level Increased by 5", player)
    ...
    # SHB Jobs (start at level 60 with crystal)
    for job in ["GNB","DNC"]:
        if state.has(f"{job} Job Crystal (default cap 60)", player):
            base_level = 60
            level_items = state.count(f"{job} Level Increased by 5", player)
    ...
    # EW Jobs (start at level 70 with crystal)
    for job in ["RPR","SGE"]:
        if state.has(f"{job} Job Crystal (default cap 70)", player):
            base_level = 70
            level_items = state.count(f"{job} Level Increased by 5", player)
    ...
    # DT Jobs (start at level 80 with crystal)
    for job in ["VPR","PCT"]:
        if state.has(f"{job} Job Crystal (default cap 80)", player):
            base_level = 80
            level_items = state.count(f"{job} Level Increased by 5", player)
    ...
    # BLU (start at level 1 with crystal)
    if state.has("BLU Job Crystal (default cap 5)", player):
        base_level = 1
        level_items = state.count("BLU Level Increased by 5", player)
        current_level = base_level + (level_items * 5)
    ...
    # DOH Jobs (start at level 1 with crystal)
    for job in ["CRP","BSM","ARM","GSM","LTW","WVR","ALC","CUL"]:
        if state.has(f"{job} Job Crystal (default cap 5)", player):
            base_level = 1
            level_items = state.count(f"{job} Level Increased by 5", player)
    ...
    # DOL Jobs (start at level 1 with crystal)
    for job in ["MIN","BTN","FSH"]:
        if state.has(f"{job} Job Crystal (default cap 5)", player):
            base_level = 1
            level_items = state.count(f"{job} Level Increased by 5", player)
    ...
    return total_levels >= target_total_int
